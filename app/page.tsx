      ...
      <div className="absolute inset-0 z-20 flex items-center justify-center text-center px-6">
        <div 
          className={`max-w-5xl transition-all duration-1000 delay-300 ${
            i === index ? "translate-y-0 opacity-100" : "translate-y-12 opacity-0"
          }`}
        >
          {s.type === "hero" ? (
            ...
            <div className="space-y-6">
              ...
              <p className="text-lg md:text-2xl text-white/90 font-medium max-w-2xl mx-auto">
                {isAr ? s.ar.subtitle : s.en.subtitle}
              </p>
              <Link 
                href={`/shop?category=${s.category}`} 
                className="inline-block bg-white text-slate-900 px-10 py-5 rounded-2xl font-black text-lg hover:bg-blue-50 transition-all shadow-2xl active:scale-95"
              >
                {t.explore}
              </Link>
            </div>
